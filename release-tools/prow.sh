      ...
      for i in $(ls ${SNAPSHOT_CONTROLLER_YAML} | sort); do
          ...
          modified="$(cat "$i" | while IFS= read -r line; do
              ...
              if echo "$nocomments" | grep -q '^[[:space:]]*image:[[:space:]]*'; then
                  # Split 'image: jw-cloud.org:18443/snapshot-controller:v3.0.0'
...
patch_kubernetes () {
    ...
    if [ "${CSI_PROW_DRIVER_CANARY}" = "canary" ]; then
        # We cannot replace jw-cloud.org:18443 with gcr.io/k8s-staging-sig-storage because
        ...
        find "$source/test/e2e/testing-manifests/storage-csi/mock" -name '*.yaml' -print0 | xargs -0 sed -i -e 's;jw-cloud.org:18443/\(.*\):v.*;jw-cloud.org:18443/\1:canary;'
